...
초등 사고력 수학 적응형 학습 앱. 아이가 문제를 풀면 정답 여부와 누적 실력 데이터를 바탕으로 다음에 풀 문제를 자동 추천한다. 부모에게는 학습 리포트를 제공한다.

- **타깃**: 초등 사고력 수학 — **학년 구분 없이 난이도 1~7**로만 수준을 나눈다 (최상위 6·7은 경시·올림피아드급; 학년이 아니라 난이도 폭)
- **문제 유형**: 연습장이 필요한 다단계 사고력 문제(퍼즐·추론·전략). 계산 드릴 금지 — 계산은 사고의 도구일 뿐
- **사용자**: 아이(문제 풀이) / **결제자**: 부모(리포트·인센티브)
