...
- **타깃**: **초등부터 성인까지 — 학년이 아니라 '사고력 난이도 폭'(1~N) 한 축으로 이어진다.** 1은 유아·초등 입문, 상위는 중·고·경시·대학·올림피아드급. 엔진(사고력＋적응형＋통계)은 학년을 가리지 않으므로, **천장(`Difficulty.MAX`)을 차근차근 올리고 상위 콘텐츠를 채워** 확장한다(현재 상한 9). 시작은 초등에서 했지만 지향은 전 연령. **천장을 한 칸 올릴 때는 그 층을 4개 영역(수와연산·변화와관계·도형과측정·자료와가능성) 모두 채워 균형을 맞춘다.**
