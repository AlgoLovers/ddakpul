...
class OfflineTts(
    ...
    // NOTE(ddakpul): 원본은 여기서 System.loadLibrary("sherpa-onnx-jni")로 APK 내장 .so를 로드했다.
    // 딱풀은 .so를 APK에 넣지 않고 런타임에 받아 filesDir에서 쓰므로, 네이티브 로드는
    // NeuralSpeechEngine이 이 클래스 사용 전에 System.load(다운로드한_경로)로 직접 수행한다.
}
